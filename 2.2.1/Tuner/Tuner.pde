...
//  println(fft.specSize());
  float maxAmp = -1;
  int maxIndex = -1;
  int minBand = fft.freqToIndex(55);
  for(int i = minBand; i < fft.specSize(); i++)
  {
  ...
  if(maxIndex >= 0 && maxAmp > 3) {
    float maxFreq = fft.indexToFreq(maxIndex);
    println("Found max freq of amp "+maxAmp+" at freq "+maxFreq+" (band "+maxIndex+")!");
    int ki = (int)round(freqToKeyIndex(maxFreq));
    String name = keyIndexToName(ki);
    
    fill(255); textAlign(RIGHT, TOP); 
    text("name: "+name+" ("+ki+", "+maxFreq+")", width, 0);
  }
