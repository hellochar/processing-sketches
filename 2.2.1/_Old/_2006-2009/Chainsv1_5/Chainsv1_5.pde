...
final int mouseDrag = 5;
final int mouseCreate = 6;
final int mouseRemove = 7;
final int mouseAnchor = 8;


final int mouseConnect = 15;
final int mouseBreak = 16;
final int mouseConnectAll = 17;

int leftMouseMode = mouseDrag;
int rightMouseMode = mouseConnect;

final int vertexInitNum = 2;
s
class makeGUI extends JFrame implements ActionListener, ItemListener, PropertyChangeListener {


  final String playPauseButtonString = "Play/Pause";
  final String leftMouseLabelString = "Left Mouse Button";
  final String leftMouseDragString = "Drag";
  final String leftMouseCreateString = "Create";
  final String leftMouseRemoveString = "Remove";
  final String leftMouseAnchorString = "Anchor";


  final String rightMouseLabelString = "Right Mouse Button";
  final String rightMouseConnectString = "Connect";
  final String rightMouseConnectAllString = "Connect All";
  final String rightMouseBreakString = "Break";


  final String showBgButtonString = "Background";
  final String showConnectionsButtonString = "Connections";
  final String showVerticesButtonString = "Vertices";
  final String applyMomentumButtonString = "Apply Momentum";
  final String rubberbandLikeButtonString = "Rubberband-like";
  final String autoConnectButtonString = "Autoconnect";
  final String wallNorthButtonString = "North Wall";
  final String wallSouthButtonString = "South Wall";
  final String wallEastButtonString = "East Wall";
  final String wallWestButtonString = "West Wall";



  final String rigidityTextFieldLabelString = "Rigidity: ";
  final String gravityTextFieldLabelString = "Gravity: ";
  final String dragTextFieldLabelString = "Momentum Drag: ";
  final String wallReboundTextFieldLabelString = "Wall Rebound: ";
...
void setup() {
  ...
  vertex[0].connectTo(vertex[vertex.length-1]);
}
...
void draw() {
  ...
  if(mousePressed) {
    if(mouseButton == CENTER)
      ...
      fill(255);
      grabbed.locSetAs(tempPoint);
      ellipse(tempPoint.x, tempPoint.y, 20, 20);
    }
  }
  if(showVertices) {
    stroke(bg/3, 25);
    if(whichConnect != null) {
      fill(0);
      whichConnect.locSetAs(tempPoint);
      ellipse(tempPoint.x, tempPoint.y, 20, 20);
    }
  }
  for(a = 0; a < vertex.length; a++) {
    ...
    if(showVertices) {
      ...
      if(temp.anchor) {
        fill(0);
        ellipse(tempPoint.x, tempPoint.y, 5, 5);
      }
    }
  }
  if(play)
...
class Vertex {
  ...
  void update() {
    if(!anchor) {
      bx = dx+median(newXs, updCounter)-loc.x;
      by = dy+median(newYs, updCounter)-loc.y;
      if(applyMomentum & wallSouth & loc.y+10+by < height) by += gravity;
