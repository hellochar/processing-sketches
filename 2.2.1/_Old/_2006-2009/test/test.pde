...
String findGenRule(String symbol) {
  ...
  */
  ...
  for(int ruleEnum = genRuleLength-1; ruleEnum >= 0; ruleEnum--) {
    ...
    //find the locations of the current lesser gen symbols
  ...
  */
  ...
  while(index >= 0) {
    ...
    println("symbolCopy is now:["+symbolCopy+"]!");
...
int occurencesOf(String which, String occ) {
  if(occ.equals("")) return which.length();
  int n = 0;
