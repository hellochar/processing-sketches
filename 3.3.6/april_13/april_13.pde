...
class Particle {
  ...
  Particle ally;
  float maxSpeed = 15;
  float spaceForAlly = 30;
  float avoidEnemyPow = 10;
  float avoidEveryonePow = 45;
  ...
  void putAllyBetweenMeAndEnemy() {
    // we want the ally to be in between me and the enemy
    // so ally and enemy make a single line
    // i want to be on that line, but behind the ally
    // draw a vector enemy towards ally;
    // add ally position to that vector of length wantedSpace
    PVector awayFromEnemy = ally.pos.copy().sub(enemy.pos).normalize();
    PVector wantedPos = ally.pos.copy().add(awayFromEnemy.mult(spaceForAlly));
  ...
  void draw() {
    ...
    PVector allyOffset = ally.pos.copy().sub(pos);
    allyOffset.setMag(15);
    stroke(0);
    ellipse(pos.x + allyOffset.x, pos.y + allyOffset.y, 4, 4);
    
    PVector enemyOffset = enemy.pos.copy().sub(pos);
    enemyOffset.setMag(15);
    noStroke();
    fill(255, 160, 40);
    ellipse(pos.x + enemyOffset.x, pos.y + enemyOffset.y, 4, 4);
  }
}
