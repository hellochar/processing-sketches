class Leaf {
  ...
  class Small {
    ...
    void add(Small s) {
      ...
      // drought outside of Kinect
      cost += 20;
      
      PVector screen = s.screenCoordinates();
      int depthX = (int)map(lerp(screen.x, width/2, 0), 0, width, 0, depthImage.width);
      int depthY = (int)map(lerp(screen.y, height/2, 0), 0, height, 0, depthImage.height);
      ...
      //colorMode(HSB);
      s.myColor = lerpColor(#1D7324, #0F9960, s.distanceToRoot / 40);
      if (s.distanceToRoot / 40 > 3) {
        s.myColor = lerpColor(#0F9960, #DB2C6F, s.distanceToRoot / 40 - 3); 
      }
    }
  ...
  void drawWorld() {
    ...
    color green = #808080;
    for (Small s : world) {
      strokeWeight(log(1 + s.weight / 1.5) / 6);
    ...
    for (Small s : terminalNodes) {
      if (s.reason == ReasonStopped.Expensive) {
        strokeWeight(0.3 + s.distanceToRoot / 50);
        if (myColor == 0) {
          if (random(4) < 1) {
            myColor = #48AFF0;
          } else if (random(3) < 1) {
        ...
        //stroke(myColor);
        stroke(s.myColor, 128 + s.distanceToRoot / 10);
        //stroke(#77c063);
        s.draw();
      }
    }
