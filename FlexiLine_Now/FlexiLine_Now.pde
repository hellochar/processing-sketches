...
//  if(mouseX > 0 & mouseX < width & mouseY > 0 & mouseY < height) {
  if(mousePressed & mouseButton != CENTER) {
    ...
    for(a = 0; a < num; a++) {
      ...
      float dist = dist(0, 0, mouseY-y[a], mouseX-x[a]);
//      dx[a] += k*cos(ang);
//      dy[a] += k*sin(ang);
      dx[a] += k*(mouseX-x[a])/dist;
      dy[a] += k*(mouseY-y[a])/dist;
    }
  }
//  }
  loadPixels();
...
int colorFor(int a) {
  colorMode(HSB);
  return color(map(atan2(dy[a], dx[a]), -PI, PI, 0, 255), 255, 128+(10*dist(0, 0, dx[a], dy[a]))%255);
}
